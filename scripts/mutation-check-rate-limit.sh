...
MUTANTS=(
  ...
  '保持窗口取最小值而不是最大值////'"$DETECTOR"'////s/RATE_LIMIT_HOLD_FLOOR_SECS\n        .max(cooldown)/RATE_LIMIT_HOLD_FLOOR_SECS.min(cooldown)/'
  ...
  '上游拒绝改成敲字////'"$DETECTOR"'////s/InterruptReason::RateLimited | InterruptReason::UpstreamRejected => ResumeTactic::Wait,/InterruptReason::RateLimited => ResumeTactic::Wait, InterruptReason::UpstreamRejected => ResumeTactic::Nudge,/'
  '兜底形状识别整段删掉////'"$DETECTOR"'////s/if let Some(shape) = rate_limit::upstream_rejection(\&lower) {/if let Some(shape) = None::<rate_limit::RejectionShape> {/'
  ...
  '状态码表里去掉 429////'"$RATE_LIMIT"'////s/&\["429", "529", "503", "502", "504"\]/\&["529", "503", "502", "504"]/'
  '把 500 也算成限流形状////'"$RATE_LIMIT"'////s/&\["429", "529", "503", "502", "504"\]/\&["429", "529", "503", "502", "504", "500"]/'
  '中文限流说法整组删掉////'"$RATE_LIMIT"'////s/"上游负载",/"__never__",/'
  ...
  '[等价] 词边界检查用 contains 替代////'"$RATE_LIMIT"'////s/super::contains_keyword(lower_errors, phrase)/lower_errors.contains(phrase)/'
)
