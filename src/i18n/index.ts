...
const TABLE = {
  // ── 外壳 ──
  "app.subtitle": ["AI Agent 守护与自动续跑", "AI agent guardian & auto-resume"],
  ...
  "stats.activity_desc": ["每天的中断检测与续跑次数", "Detections and resumes per day"],
  ...
  "cost.trend_desc": ["近 {days} 天的 token 花费", "Token spend over the last {days} days"],
  "cost.projects": ["项目排行", "Top projects"],
  "cost.projects_desc": ["近 {days} 天按项目聚合", "Grouped by project, last {days} days"],
  "cost.no_data": [
  ...
  "cost.window_used": ["{hours} 小时窗口已用 {percent}%", "{percent}% of the {hours}h window used"],
  "cost.minutes_left": ["约 {minutes} 分钟后触发限流", "about {minutes} min to the limit"],
  ...
  "cost.range_total": ["近 {days} 天合计 ${cost}", "${cost} over the last {days} days"],
  "cost.window_tokens": ["{used} / {budget} tokens", "{used} / {budget} tokens"],

  // ── 会话历史时间线 ──
  "history.title": ["会话历史", "Session history"],
  "history.desc": ["按项目或终端找回以前的会话", "Find past sessions by project or terminal"],
  ...
  "cfg.detection.desc": ["多久看一眼、多久算卡住", "How often we look, and when we call it stuck"],
  ...
  "cfg.behavior.desc": ["判定中断之后做什么", "What happens once a stall is called"],
  ...
  "cfg.startup_scan.desc": ["打开应用后自动开始监控", "Begin monitoring as soon as the app opens"],
  ...
  "cfg.notify.desc": ["会话需要你的时候怎么叫你", "How you get pulled in when a session needs you"],
  ...
  "cfg.notify_sound.desc": ["提醒时顺带响一下", "Chime alongside the notification"],

  "cfg.notify_volume": ["音量 {value}%", "Volume {value}%"],
  "cfg.notify_throttle": ["同一会话最短提醒间隔（秒）", "Minimum gap between alerts (seconds)"],
  ...
  "cfg.cost.desc": ["token 统计与预算告警", "Token accounting and budget alerts"],
  ...
  "cfg.daily_budget": ["每日预算（美元，0 = 不限）", "Daily budget (USD, 0 = off)"],
  "cfg.session_budget": ["单会话预算（美元，0 = 不限）", "Per-session budget (USD, 0 = off)"],
  "cfg.alert_percent": ["用到预算的多少 % 告警", "Alert at this % of budget"],
  "cfg.rate_window": ["限流窗口（小时）", "Rate limit window (hours)"],
  "cfg.rate_budget": ["窗口内 token 额度（0 = 不预测）", "Tokens per window (0 = no forecast)"],

  // ── 设置：提示词 ──
  "cfg.prompts": ["续跑提示词", "Resume prompts"],
  "cfg.prompts.desc": ["中断后替你敲进去的那句话", "What we type into the agent when it stalls"],
  "cfg.generic_prompt": ["通用续跑提示词", "Generic resume prompt"],
  "cfg.goal_prompt": ["目标恢复提示词", "Goal recovery prompt"],
  "cfg.goal_badge": ["检测到活跃目标时自动使用", "Used when a live goal is detected"],
  ...
  "cfg.ai.desc": ["让模型看一眼输出，减少误判", "Let a model read the output to cut false alarms"],
  ...
  "cfg.adapter_session": ["会话文件模式（可选）", "Session file pattern (optional)"],
  ...
  "cfg.remote_token.hint": ["留空则启动时自动生成", "Generated automatically if left blank"],
  ...
  "cfg.system.desc": ["托盘、开机自启与语言", "Tray, launch at login, and language"],
...
export function useI18n() {
  ...
  return useMemo(
    () => ({ lang, t: (key: I18nKey, vars?: Vars) => translate(lang, key, vars) }),
    [lang]
  );
}
