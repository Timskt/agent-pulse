...
**③ 这类 bug 单元测试抓不到。** 时刻比较和形状识别都能测（65 个测试），但
「日志一直在长、证据滚走了、它还按着」需要一个真实增长的记录文件，只能实机验
...
| `scripts/mutation-check-rate-limit.sh` | 19 | 限流形状识别、等待时间解析、保持窗口（见 12.8） |
...
还学到第三条：**sed 表达式没匹配到任何东西的变异体，必须和「变异体活下来」区分报告。**
第一版有 8 个变异体的 sed 是照记忆写的、跟真实源码对不上，改了个空气然后报「测试通过」——
那不是绿灯，那是没测。现在脚本会把这种情况单独报成「这条检查是假的」。
